...
function pickPrimaryImageUrl(obj) {
  ...
  if (!Array.isArray(imgs) || !imgs.length) return "/images/section/box-house.jpg";
  const first = imgs[0];
...
function pickPriceAndOperation(obj) {
  ...
  return { price, currency, operation };
}
...
export function tokkoObjectToListingDto(raw) {
  ...
  const title = String(
    obj.publication_title || obj.description || "Sin título",
  ).slice(0, 220);
